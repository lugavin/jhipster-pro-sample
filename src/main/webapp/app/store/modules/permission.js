...
function hasPermission(permission, route) {
  if (route.meta && route.meta.authorities) {
    ...
    for (let i = 0, len = permission.length; i < len; i++) {
      flag = route.meta.authorities.includes(permission[i]);
      if (flag) {
        return true;
      }
    }
  ...
  actions: {
    GenerateRoutes({ commit }, data) {
      return new Promise(resolve => {
        const { authorities, viewPermissions } = data;
        const accessedRouters = filterAsyncRouter(viewPermissions, authorities);

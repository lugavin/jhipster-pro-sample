...
<template>
  <a-form-model ref="dynamicValidateForm" layout="inline" :model="dynamicValidateForm">
    <a-table
      ...
      bordered
      :scroll="{
        x: record.list.length * 190 + 80 + (!record.options.hideSequence ? 60 : 0),
    ...
    >
      ...
      <template slot="dynamic-delete-button" slot-scope="text, record">
        <a-icon v-if="!disabled" class="dynamic-delete-button" type="minus-circle-o" @click="removeDomain(record)" />
      </template>
    </a-table>

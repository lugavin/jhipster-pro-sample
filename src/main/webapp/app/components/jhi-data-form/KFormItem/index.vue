...
<template>
  <a-form-item
    ...
    :label="record.label"
    :label-col="
      formConfig.layout === 'horizontal' ? (record.options.labelColSpan ? { span: record.options.labelColSpan } : formConfig.labelCol) : {}
    "
    :wrapper-col="
      formConfig.layout === 'horizontal'
        ? record.options.labelColSpan
          ? { span: 24 - record.options.labelColSpan }
  ...
  >
    ...
    <a-input-number
      ...
      :precision="
        record.options.precision > 50 || (!record.options.precision && record.options.precision !== 0) ? null : record.options.precision
      "
      :placeholder="record.options.placeholder"
      @change="handleChange($event, record.model)"
      v-decorator="[
    ...
    >
      <a-select-option
        v-for="option in !record.options.dynamic
          ? record.options.options
          : dynamicData[record.options.dynamicKey]
          ? dynamicData[record.options.dynamicKey]
          : []"
        :value="option[record.options.valueField]"
        :key="option[record.options.valueField]"
      >
  ...
  <!-- 可隐藏label -->
  <a-form-item
    v-else-if="record.type === 'batch' || record.type === 'editor' || record.type === 'codemirror'"
  ...
  >
    ...
    <!-- 代码编辑器 -->
    <codemirror
      v-else-if="record.type === 'codemirror'"
      ref="KCodeMirror"
      :style="`width:${record.options.width}`"
      :options="{ mode: record.options.mode, lineNumbers: record.options.lineNumbers, lineWrapping: record.options.lineWrapping }"
  ...
  <a-form-item
    ...
    :wrapper-col="formConfig.layout === 'horizontal' ? { ...formConfig.wrapperCol, offset: formConfig.labelCol.span } : {}"
  >
    <a-button
      :disabled="disabled || record.options.disabled"
      @click="
...
import SingleUploadImg from '../SingleUploadImage';
  ...
  methods: {
    evil(fn) {
      if (fn && this.myForm && Object.keys(this.myForm).length > 0) {
        // fn内容例子：this.myForm.getFieldValue('id')

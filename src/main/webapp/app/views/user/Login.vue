<template>
  <div class="main">
    <a-form id="formLogin" class="user-layout-login" ref="formLogin" :form="form" @submit="handleSubmit">
      ...
      <a-form-item>
        <a-checkbox v-decorator="['rememberMe', { valuePropName: 'checked', initialValue: false }]">自动登录</a-checkbox>
...
export default {
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          ...
          const apiUrl = customActiveKey === 'tab1' ? 'api/authenticate' : 'api/authenticate/mobile';
          const postData =
            customActiveKey === 'tab1'
              ? loginParams
              : { mobile: loginParams.mobile, code: loginParams.captcha, rememberMe: loginParams.rememberMe, imageCode: '' };
          localStorage.removeItem('jhi-authenticationToken');
          sessionStorage.removeItem('jhi-authenticationToken');
          Axios.post(apiUrl, postData).subscribe(
            res => {
    ...
    getCaptcha(e) {
      ...
      validateFields(['mobile'], { force: true }, (err, values) => {
        if (!err) {
          ...
          this.getSmsCode(values.mobile).subscribe(
            res => {
    ...
    getSmsCode(mobile, imageCode = '') {
      return Axios.get(`api/mobile/smscode?imageCode=${imageCode}&mobile=${mobile}`);
    },
  },
};
</script>

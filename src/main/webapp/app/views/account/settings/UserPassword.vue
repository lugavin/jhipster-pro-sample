    ...
    handleOk() {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          Axios.post('api/account/change-password', { currentPassword: params.oldpassword, newPassword: params.password }).subscribe(
            res => {
              that.$message.success('密码更新成功。');
              that.close();
            },
            error => {
              that.$message.warning('密码更新失败！');
            },
            () => {

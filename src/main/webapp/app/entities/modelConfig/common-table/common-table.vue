  ...
  <a-card :bordered="false">
    <a-row :gutter="16">
      ...
      <a-col :span="24 - filterTreeSpan">
        <a-skeleton :loading="xGridColumns.length === 0">
          <vxe-grid
            border
            show-overflow
            tree-config
            ...
            :pager-config="xGridPagerConfig"
            :sort-config="xGridSortConfig"
          ...
          >
            ...
            <template v-slot:toolbar_buttons>
              <a-row class="toolbar_buttons_xgrid" :gutter="16">
                ...
                <a-col :lg="14" :md="10" :sm="11" align="right">
                  ...
                  <a-dropdown>
                    <a-menu slot="overlay" @click="handleConditionMenuClick">
                      <a-menu-item :key="-1"> <a-icon type="undo" />恢复默认</a-menu-item>
                      <a-menu-divider />
                      <a-menu-item :key="item.id" v-for="item in commonConditions"> <a-icon type="filter" />{{ item.name }}</a-menu-item>
                      <a-menu-divider />
            ...
            <template slot-scope="{ row, column }" slot="recordAction">
              ...
              <a-tooltip placement="top" title="表单设计">
    ...
    <a-modal
      title="新增或编辑"
      :visible="showCommonConditionModal"
      :destroyOnClose="true"
      :footer="null"
      @cancel="updateCommonConditionModalCancel"
      width="85%"
      :maskClosable="false"
    >
      <jhi-common-condition-update @cancel="updateCommonConditionModalCancel" :showInModal="true" :updateEntityId="null" />
    </a-modal>
  </a-card>
</template>

<template>
  <a-card :bordered="false">
        ...
        <a-skeleton :loading="xGridColumns.length === 0">
          ...
          >
            ...
            <template slot-scope="{ row, column }" slot="recordAction">
              ...
              <a-tooltip placement="top" title="增加下级">
                <a-button type="primary" shape="circle" size="small" @click="addChild(row)">
                  <a-icon type="enter"></a-icon>
                </a-button>
              </a-tooltip>
            </template>
          </vxe-grid>
    ...
    <a-modal
      ...
      :destroyOnClose="true"
      :footer="null"
      @cancel="updateModalCancel"
      width="85%"
      :maskClosable="false"
    >
      <jhi-view-permission-update @cancel="updateModalCancel" :showInModal="true" :updateEntityId="viewPermissionId" :parentId="parentId" />
    </a-modal>
  </a-card>
</template>

<script lang="ts" src="./view-permission.component.ts"></script>
<style>
